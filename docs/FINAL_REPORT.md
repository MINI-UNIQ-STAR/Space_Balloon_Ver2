...
- **복구 메커니즘 (Recovery Strategy)**:
  ...
  2.  **Level 2 (Soft Reset)**: I2C 버스 복구 (9-Clock Pulse) 및 드라이버 레벨 초기화. ✅ **구현 완료 (2026-01-09)**
  3.  **Level 3 (Hard Reset)**: **P-MOS Load Switch**를 OFF/ON 하여 물리적 전원 재인가 (Latch-up 해제). ✅ **구현 완료 (2026-01-09)**
  4.  **Level 4 (Isolation)**: 5회 이상 복구 실패 시 해당 센서를 `Disabled` 처리하여 시스템 버스 보호. ✅ **구현 완료**

**구현 현황 (2026-01-09 업데이트)**:
- ✅ 타임아웃 감지 및 상태 머신: 완전히 구현됨 (Core/Src/fdir.c:97-184)
- ✅ 온도 기반 센서 보호: 완전히 구현됨 (PMS3003, CM1107N 저온 차단)
- ✅ 범위 검증 및 연속성 검사: 완전히 구현됨 (고도 점프 감지 등)
- ✅ **하드웨어 복구 로직**: `Sensors_Reset()` 9개 센서 모두 구현 완료 (110 LOC, Core/Src/sensors.c:238-348)
- ✅ **I2C 버스 복구**: BSP_I2C1_Recovery(), BSP_I2C3_Recovery() 구현 완료 (104 LOC, Core/Src/bsp.c:177-280)
- ✅ **단위 테스트**: FDIR (4/4), Kalman (4/4), PID (5/5) 모두 PASS
- ✅ **통합 테스트**: HostSim (SITL) 정상 동작 확인

> **검증 현황**: 소프트웨어 레벨 검증 완료 (단위/통합 테스트). 실제 P-MOS 전원 사이클 및 GPIO 리셋 동작은 실제 STM32 하드웨어에서 추가 검증이 필요합니다.
...
### 5.1 하드웨어 통합 테스트 (우선순위: 최고)
- **목표**: 실제 STM32G431CBU6 보드와 센서들로 구성된 시스템에서 전체 기능 검증
- **범위**:
...
| **프로젝트 매니저** | **Seyeong Choi** | 2026-01-19 |
