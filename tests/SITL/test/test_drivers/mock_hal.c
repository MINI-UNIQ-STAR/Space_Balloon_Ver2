...
MockI2C_LastWrite_t *MockI2C_GetLastWrite(void) { return &last_i2c_write; }
...
void HAL_Delay(uint32_t Delay) { current_tick += Delay; }

/** @brief 시간 강제 전진 (Test Helper) */
void MockHAL_AdvanceTick(uint32_t ms) { current_tick += ms; }
...
MockUART_LastTx_t *MockUART_GetLastTx(void) { return &last_uart_tx; }
...
/** @brief 센서 리셋 상태머신 처리 (Mock) */
void Sensors_ProcessReset(void) {
  printf("MOCK FDIR: Sensors_ProcessReset called\n");
}

/** @brief TIM PWM 제어 (Mock) */
void __HAL_TIM_SET_COMPARE(TIM_HandleTypeDef *htim, uint32_t Channel,
                           uint32_t Compare) {
  // printf("MOCK TIM SET COMPARE: Channel=%u, Compare=%u\n", Channel, Compare);
}
