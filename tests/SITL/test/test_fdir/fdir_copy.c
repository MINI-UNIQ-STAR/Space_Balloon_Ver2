...
static const uint8_t sensor_max_recovery[SENSOR_ID_COUNT] = {
    ...
    [SENSOR_ID_SHT] = 2,  [SENSOR_ID_RAD] = 2, [SENSOR_ID_EXT_TEMP] = 2,
};
...
void FDIR_Update(void) {
  ...
  for (i = 0U; i < (uint8_t)SENSOR_ID_COUNT; i++) {
    ...
    if (current_ext_temp_x100 < min_temp && !sensor_cold_disabled[i]) {
      ...
      sensors_health[i].state =
          FDIR_STATE_WARNING; // Mark as warning, not permanent failure
#ifdef DEBUG
      printf("FDIR: Sensor %d COLD DISABLED (%.1f°C < %.1f°C)\n", i,
             current_ext_temp_x100 / 100.0f, min_temp / 100.0f);
#endif
    ...
    if (diff > timeout) {
      ...
      printf("FDIR: Sensor %d Timeout (%lu ms). Recovery %lu/%d\n", i,
             (unsigned long)diff,
             (unsigned long)sensors_health[i].recovery_count + 1,
             sensor_max_recovery[i]);
#endif
...
void FDIR_UpdateGPSAltitude(float gps_alt_m) {
  if (FDIR_ValidateRange_GPS_Alt(gps_alt_m) &&
      FDIR_CheckContinuity_GPS_Alt(gps_alt_m)) {
...
void FDIR_UpdateBaroAltitude(float baro_alt_m) {
  ...
  // This is just for internal tracking, main range check should be on raw
  // pressure
...
uint16_t FDIR_GetStatusFlags(void) {
  ...
  if (flags & (STATUS_GPS_WARN | STATUS_BARO_WARN | STATUS_IMU_WARN |
               STATUS_TEMP_WARN)) {
    flags &= ~STATUS_SYS_OK;
  }
