 ...
 * @brief Define the basic structure of the DFRobot_OzoneSensor class, the
...
DFRobot_OzoneSensor::DFRobot_OzoneSensor(TwoWire *pWire) {
  this->_pWire = pWire;
  this->_addr = 0; // Initialize address
}

DFRobot_OzoneSensor::~DFRobot_OzoneSensor() { this->_pWire = NULL; }
...
int16_t DFRobot_OzoneSensor::readOzoneData(uint8_t collectNum) {
  ...
  if (collectNum > 0) {
    ...
    if (_M_Flag == 0) {
      ...
      delay(100); // read passive data in passive mode, first request once, then
                  // read the data
