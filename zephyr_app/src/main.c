...
static telemetry_payload_t sensor_data;
static uint32_t sensor_update_count = 0;
...
extern void Sensors_Read_AirQuality(uint16_t *co2, int16_t *ozone,
...
static void sensor_thread_fn(void *p1, void *p2, void *p3) {
  ...
  while (1) {
    ...
    /* 실제 센서에서 데이터 읽기 */
    Sensors_Read_IMU(sensor_data.accel_mps2_x1000, sensor_data.gyro_rads_x1000);
    Sensors_Read_Baro(&sensor_data.ms5611_press_pa,
                      &sensor_data.ms5611_temp_c_x100);
    Sensors_Read_Humid(&sensor_data.sht31_temp_c_x100,
                       &sensor_data.sht31_rh_x100);
    Sensors_Read_External(&sensor_data.external_temp_c_x100);
    Sensors_Read_BoardTemp(&sensor_data.board_temp_c_x100);
    Sensors_Read_Battery(&sensor_data.bat_mv, &sensor_data.bat_temp_c_x100);

    /* 공기질 센서 */
    Sensors_Read_AirQuality(&sensor_data.co2_ppm, &sensor_data.ozone_ppb,
                            &sensor_data.pm1_ugm3, &sensor_data.pm25_ugm3);
    /* Note: Sensors_Read_AirQuality doesn't return PM10 yet, just copy PM2.5 or
     * leave 0 */
    sensor_data.pm10_ugm3 = sensor_data.pm25_ugm3;

    /* 방사선 센서 */
    Sensors_Read_Rad(&sensor_data.gdk101_usvh_x100);

    /* 고도 계산 */
    sensor_data.press_alt_m = calculate_altitude(sensor_data.ms5611_press_pa);
    sensor_data.kf_alt_m =
        sensor_data.press_alt_m; /* Kalman filter not fully integrated yet */

    /* 온도 보호 */
    FDIR_CheckTemperatureProtection(sensor_data.ms5611_temp_c_x100);

    /* FDIR 범위 검증 */
    FDIR_ValidateRange_Baro(sensor_data.ms5611_press_pa);

    /* 상태 플래그 확인 */
    uint16_t flags = FDIR_GetStatusFlags();

    /* 주기적 로그 (1초마다) */
    if (sensor_update_count % 10 == 0) {
      LOG_INF("Sensors: accel=[%d,%d,%d], press=%uPa, alt=%.1fm, temp=%.1fC, "
              "flags=0x%04X",
              sensor_data.accel_mps2_x1000[0], sensor_data.accel_mps2_x1000[1],
              sensor_data.accel_mps2_x1000[2], sensor_data.ms5611_press_pa,
              sensor_data.press_alt_m, sensor_data.ms5611_temp_c_x100 / 100.0f,
              flags);
    }
...
static void telemetry_thread_fn(void *p1, void *p2, void *p3) {
  ...
  while (1) {
    ...
    /* Update telemetry specific fields */
    sensor_data.uptime_ms = now;
    sensor_data.status_flags = flags;

    /* UART3로 148바이트 프레임 전송 */
    Telemetry_Process(&sensor_data);
...
int main(void) {
  ...
  /* 스레드 시작 */
  k_thread_create(&fdir_thread, fdir_stack, K_THREAD_STACK_SIZEOF(fdir_stack),
                  fdir_thread_fn, NULL, NULL, NULL, 5, 0, K_NO_WAIT);

  k_thread_create(&sensor_thread, sensor_stack,
                  K_THREAD_STACK_SIZEOF(sensor_stack), sensor_thread_fn, NULL,
                  NULL, NULL, 4, 0, K_NO_WAIT);

  k_thread_create(&telemetry_thread, telemetry_stack,
                  K_THREAD_STACK_SIZEOF(telemetry_stack), telemetry_thread_fn,
