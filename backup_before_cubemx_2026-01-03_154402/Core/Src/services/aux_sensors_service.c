...
static int16_t s_temp_c_x100 = 0;
...
void aux_sensors_service_tick(uint32_t now_ms)
{
	...
	if (!s_temp_conv_pending) {
		...
		if ((uint32_t)(now_ms - s_temp_conv_start_ms) >= 100u) {
			ds18b20_reading_t r;
			if (ds18b20_read_temperature(&r) && r.valid) {
				// Spec note: 85.0C can be a power-on reset default; treat as invalid.
				if (r.temp_c_x100 != 8500) {
					s_temp_c_x100 = r.temp_c_x100;
					s_temp_valid = true;
