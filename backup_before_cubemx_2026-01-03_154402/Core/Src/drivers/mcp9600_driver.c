...
enum {
	...
	MCP9600_REG_COLD_JUNCTION = 0x02,
	MCP9600_COLD_JUNCTION_LSB_C_X100_NUM = 25, // 0.0625C = 25/400 C => 25/4 centiC
	MCP9600_COLD_JUNCTION_LSB_C_X100_DEN = 4,
};
...
static int32_t convert_raw_to_c_x100(int16_t raw)
{
	...
	int32_t scaled = (int32_t)raw * MCP9600_COLD_JUNCTION_LSB_C_X100_NUM;
	if (scaled >= 0) {
		return (scaled + (MCP9600_COLD_JUNCTION_LSB_C_X100_DEN / 2)) / MCP9600_COLD_JUNCTION_LSB_C_X100_DEN;
	}
	return (scaled - (MCP9600_COLD_JUNCTION_LSB_C_X100_DEN / 2)) / MCP9600_COLD_JUNCTION_LSB_C_X100_DEN;
}
